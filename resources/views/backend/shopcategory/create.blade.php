...
@section('title')
{{ isset($item) ? 'Edit' : 'Add' }} Shp Category
@endsection
...
<div class="add_food_iten_box">
    <h2>Add New {{ $title }}</h2>
    <form id="uploadForm" action="{{ isset($item) ? route('admin.shopcategory.store', $item->id) : route('admin.shopcategory.store') }}" method="post" enctype="multipart/form-data">
        @csrf
